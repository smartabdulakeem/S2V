...
Three further defects were measured in a real composed prompt (`islamic_history`, a shot from
`THE_BATTLE_OF_THE_MUD`), all of which this spec now also fixes:

- **The world anchor is emitted twice.** `world_anchor` is "7th century Arabian Peninsula, early
  Islamic era", and that pack's `style_block` *ends* with "historically accurate 7th century
  Arabian Peninsula, early Islamic era". `compose_gap_prompt` appends both, every time.
- **`scene_from_narration` truncates mid-phrase.** It hard-cuts at 34 words
  (`pipeline/library.py:1219`), producing prompts that end `"...sank to the knee in churned,"` --
  the noun lost. Every narration longer than 34 words is chopped this way.
- **The scene half is narration, not image direction.** "rode through the night with the advance
  guard, reaching the Jordan valley before dawn" is story. A generator cannot photograph
  "reaching". The result is 84 words carrying very little the generator can act on: long, but thin.
...
### Printed-matter presets

`newsprint_profile`, `newspaper_archive` and `propaganda_poster` describe printed artefacts.
Since negative blocks are not emitted by default (see 3d), there is no live contradiction with
"no text, no watermark" -- but the prose must still describe these as *texture and composition*
and never request legible lettering, because generators render requested text badly. If one
proves unusable in practice, cut that preset rather than reintroducing a negative block.
...
Two changes: the preset replaces `style_block`, and the scene half is rebuilt as structured
image direction rather than a dump of narration.

### 3a. The slot system

`compose_gap_prompt` (`pipeline/library.py:1254`) assembles the prompt from named slots, in this
fixed order. A slot that resolves to nothing is omitted, never left empty.
...
Each match table is a list of `(regex, phrase)` pairs held in one module-level constant per slot,
so the vocabulary is editable in one place without touching logic.
...
`scene_from_narration`'s raw 34-word cut is replaced by the slot system, which never quotes
narration verbatim and therefore cannot truncate mid-phrase. Where narration text is still
needed, cutting happens at a clause or sentence boundary, never mid-phrase.

### 3c. Preset replaces style_block

- a preset resolves from the pack -> emit its prose as the Medium slot
- no preset, or an unknown key -> emit `style_block`, exactly as today

The Setting slot is suppressed when `world_anchor` already appears in the Medium text, which is
what removes the duplication.

The three call sites (`library.py:1531`, `1563`, `1609`) pass `style_preset` through, sourced
from `project_info.get("visual_type")`.

### 3d. Negative prompts

**Already correct, no change.** `include_negative` defaults to
`_setting("include_negative_prompt", False)`, i.e. off (`pipeline/library.py:1305`). The code
comment records why: the negative text was being pasted into single-box image tools as though it
were part of the description, "asking for the very things it meant to forbid." Negative blocks
stay in the packs, stay off by default, and are never emitted into copied prompts.
